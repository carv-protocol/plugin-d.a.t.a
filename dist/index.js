// src/providers/ethereum/ethereumData.ts
import {
  elizaLogger,
  ...
  async analyzeQuery(queryResult, message, runtime, state) {
    try {
      ...
      if (analysisResponse?.object) {
        const obj = analysisResponse.object;
        const analysisText = [
          "Summary:",
          obj.summary,
          "",
          ...(obj.analysis || []).map((item) => [`${item.aspect}:`, item.content, ""]).flat(),
